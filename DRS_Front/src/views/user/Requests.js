...
import RequestNavbar from "components/Navbars/RequestNavbar.js";
...
const handleReject = async (userId) => {
  try {
    const token = localStorage.getItem("DRS_user_token");
    await axios.post(`${process.env.REACT_APP_API_URL}friends/${userId}/reject`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
    setRequests((prev) => prev.filter((r) => r.user_id !== userId));
  } catch (err) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {requests.map((req) => (
              <div key={req.request_id} className="bg-white shadow p-4 rounded">
                <h4 className="font-semibold mb-1">{req.first_name} {req.last_name}</h4>
                <p className="text-sm text-gray-500 mb-2">@{req.username}</p>
